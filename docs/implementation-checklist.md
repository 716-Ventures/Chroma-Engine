...
- [ ] Remove vendored legacy media binaries once Chroma Engine covers the required native paths. Still intentionally open: native Rust playback does not yet cover every required fallback path, including DTS/TrueHD-style audio and other browser-unsafe movie variants that currently need legacy transcode/remux support.
