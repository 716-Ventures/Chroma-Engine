...
- [ ] Linux executable VAAPI/NVENC/QSV path. H.264 and HEVC Main/Main10 VA-API decode plus H.264 and HEVC Main VA-API encode are executable behind the release-enabled `linux-vaapi` feature. Intel iHD/i965 render nodes additionally provide separately named Quick Sync H.264/HEVC decode and Main encode sessions after vendor and codec verification, without a link-time oneVPL dependency. Retained stateless sessions use page-aligned NV12/P010 user-pointer surfaces, preserve timing, produce fMP4-ready length-prefixed samples and avcC/hvcC, and fall back to portable codecs. Retained NVENC H.264/HEVC Main encode and NVDEC H.264/HEVC Main decode are executable behind the release-enabled `linux-nvidia` feature; encoder profiles require real runtime encode probes and decoders require device codec-capability queries. NVDEC Main10 remains.
- [ ] Windows executable NVENC/QSV/AMF/D3D path. Retained H.264 and HEVC Main encode sessions execute through hardware NVENC, hardware Intel Quick Sync, or native Media Foundation using checked BGRA-to-NV12 upload, timing preservation, length-prefixed access units, avcC/hvcC extraction, and real encode smoke probes. Retained H.264 hardware decode now executes through a hardware-only Media Foundation transform and D3D11 NV12 textures, with checked staging readback, BGRA conversion, timing restoration, and a real round-trip smoke before advertisement. Hardware HEVC decode and AMD AMF remain modeled/opened but non-executable.
...
- [x] Keep audio routing executable across planning and segment generation. Copy-compatible tracks rank first; otherwise portable TrueHD/DTS-to-AAC bridges remain selectable, and explicit track selection remains authoritative. The DTS-first Gremlins remux still selects its AC-3 alternate by default for copy efficiency, while an explicit `a0` selects the executable DTS bridge.
- [x] Native DTS decode bridge for MKV audio tracks that have no copy-compatible alternate. The retained portable decoder feeds source-anchored AAC-LC access units into fMP4 using the same bounded Chroma-native bridge contract as TrueHD. Segment zero of the Gremlins DTS-HD MA 5.1 fixture completed in 0.23 seconds with copied 4K HEVC video, 222 AAC frames, and about 44.2 MB maximum RSS.
