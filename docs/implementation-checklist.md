...
- [ ] Windows executable NVENC/QSV/AMF/D3D path. Hardware-only Media Foundation H.264/HEVC decoder and encoder transforms are now enumerated and activated with balanced COM/MF lifetime management. Successful decoder activation reports `opened`; compressed-frame processing and output verification remain before the backend can become executable.
