...
- [ ] Linux executable VAAPI/NVENC/QSV path. H.264 VA-API decode is executable behind the release-enabled `linux-vaapi` feature: retained stateless packet submission writes directly into page-aligned NV12 user-pointer surfaces, synchronizes pictures, preserves packet timing, and falls back to OpenH264 when initialization fails. HEVC/P010 packet execution and VA-API encode remain, as do NVENC/NVDEC and QSV.
