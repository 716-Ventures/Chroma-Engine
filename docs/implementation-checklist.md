...
- [x] Remove vendored legacy media binaries once Chroma Engine covers the required native paths. GenusServer no longer tracks or requires `native/jellyfin-ffmpeg` payloads, vendor/check scripts, or a vendored FFmpeg resolver; fallback media tools now require explicit support paths or packaged resources.
