...
- [ ] Linux executable VAAPI/NVENC/QSV path. VA-API probing now dynamically opens render nodes and verifies H.264/HEVC VLD profiles with real config, surface, and decode-context allocation; compressed packet submission, decoded-frame mapping, and the executable encode path remain.
