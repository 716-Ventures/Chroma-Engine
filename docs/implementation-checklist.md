...
- [ ] Linux executable VAAPI/NVENC/QSV path. H.264 and HEVC Main/Main10 VA-API decode are executable behind the release-enabled `linux-vaapi` feature: retained stateless packet submission writes directly into page-aligned NV12/P010 user-pointer surfaces, synchronizes pictures, preserves packet timing, and falls back to the portable decoders when initialization fails. VA-API encode remains, as do NVENC/NVDEC and QSV.
