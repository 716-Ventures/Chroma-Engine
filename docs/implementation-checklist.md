...
- [ ] Linux executable VAAPI/NVENC/QSV path. VA-API probing now dynamically opens render nodes, verifies H.264/HEVC VLD profiles with real config/surface/context allocation, and attempts synchronized direct NV12/P010 image mapping; compressed packet submission, decoded-picture scheduling, and the executable encode path remain.
