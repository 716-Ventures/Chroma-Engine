...
- [ ] Windows executable NVENC/QSV/AMF/D3D path. Retained H.264 and HEVC Main encode sessions execute through hardware NVENC, hardware Intel Quick Sync, or native Media Foundation using checked BGRA-to-NV12 upload, timing preservation, length-prefixed access units, avcC/hvcC extraction, and real encode smoke probes. Retained H.264 and HEVC Main hardware decode execute through hardware-only Media Foundation transforms and D3D11 NV12 textures, with checked staging readback, BGRA conversion, timing restoration, and codec-specific round-trip smokes before advertisement. HEVC Main10 and AMD AMF remain modeled/opened but non-executable.
