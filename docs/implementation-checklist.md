...
- [ ] Linux executable VAAPI/NVENC/QSV path. H.264 and HEVC Main/Main10 VA-API decode plus H.264 VA-API encode are executable behind the release-enabled `linux-vaapi` feature. Retained stateless sessions use page-aligned NV12/P010 user-pointer surfaces, preserve timing, produce fMP4-ready AVCC/avcC, and fall back to portable codecs. HEVC encode, NVENC/NVDEC, and QSV remain.
