...
- [ ] Windows executable NVENC/QSV/AMF/D3D path. Retained H.264 and HEVC Main encode sessions now execute through hardware NVENC, hardware Intel Quick Sync, or native Media Foundation using checked BGRA-to-NV12 upload, timing preservation, length-prefixed access units, avcC/hvcC extraction, and real encode smoke probes. Hardware-only Media Foundation H.264/HEVC decoder transforms are enumerated and activated with balanced COM/MF lifetime management. Successful decoder activation reports `opened`; compressed-frame processing and output verification remain before decode can become executable. AMD AMF remains modeled.
