...
- [ ] Linux executable VAAPI/NVENC/QSV path. H.264 and HEVC Main/Main10 VA-API decode plus H.264 and HEVC Main VA-API encode are executable behind the release-enabled `linux-vaapi` feature. Retained stateless sessions use page-aligned NV12/P010 user-pointer surfaces, preserve timing, produce fMP4-ready length-prefixed samples and avcC/hvcC, and fall back to portable codecs. Retained NVENC H.264 encode is executable behind the release-enabled `linux-nvidia` feature and is advertised only after a real runtime encode succeeds. NVENC HEVC, NVDEC, and QSV remain.
