...
- [ ] Windows executable NVENC/QSV/AMF/D3D path. Retained H.264 and HEVC Main encode sessions execute through hardware NVENC, hardware Intel Quick Sync, or native Media Foundation using checked BGRA-to-NV12 upload, timing preservation, length-prefixed access units, avcC/hvcC extraction, and real encode smoke probes. Retained H.264 and HEVC Main/Main10 hardware decode execute through hardware-only Media Foundation transforms and D3D11 NV12/P010 textures, with checked staging readback, BGRA conversion, timing restoration, and codec-specific runtime gating. AMD AMF remains modeled/detected but non-executable; Windows systems without an executable vendor path retain Media Foundation and portable fallbacks.
