...
- **Portable fallback is a real backend, not a capability claim.** The source-built OpenH264 dependency provides a retained CPU H.264 encoder on macOS, Windows, Linux, and Linux-based NAS systems. It emits the same AVCC/avcC contract as the hardware encoder, while platform hardware paths remain preferred where they are executable. Non-macOS full transcode still requires portable decode and audio encode stages.
