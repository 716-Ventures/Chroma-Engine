...
- **Linux GPU integrations remain runtime optional.** VA-API and NVIDIA NVENC backends are built into Linux releases without link-time driver dependencies. Startup probes execute a small real encode before advertising a hardware encoder, and session construction falls back to portable H.264 when the device, codec profile, or runtime libraries are unavailable. This keeps headless NAS and non-GPU Linux installations on the same release artifact.
