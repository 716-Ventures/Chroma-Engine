...
- **Portable fallback is a real backend, not a capability claim.** The source-built OpenH264 dependency provides retained CPU H.264 decode and encode on macOS, Windows, Linux, and Linux-based NAS systems. It uses the same BGRA and AVCC/avcC contracts as the hardware path, while platform hardware remains preferred where executable. Non-macOS HEVC input and audio that cannot stay compressed still require additional portable stages.
