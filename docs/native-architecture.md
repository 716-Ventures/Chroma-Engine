...
- **Windows H.264/HEVC Main hardware decode crosses a checked D3D11 boundary.** The retained Media Foundation session requests a hardware-only decoder on a Chroma-owned D3D11 video device. HEVC hvcC parameter sets and length-prefixed access units are converted to Annex B before submission. Each returned NV12 texture is copied to a same-device staging resource, mapped with its reported row pitch, converted to BGRA through the shared checked converter, and matched back to source timing with internal tokens. A generated frame must survive encode, decode, readback, and conversion before D3D11VA is advertised; OpenH264 or the portable HEVC Main/Main10 decoder remains the fallback. Chroma vendors the narrow Mediaway decoder facade needed for this path at v0.1.4 so the crate retains its Rust 1.90 minimum.
