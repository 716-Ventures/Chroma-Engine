...
//!
//! Windows x86/x64 only: other targets, including Windows ARM64, use the
//! [`EncodeError::Unsupported`] stub so automatic selection can try another backend.
...
impl NvencVideoEncoder {
    ...
    /// Unsupported operating system or architecture: NVENC backend unavailable.
    #[cfg(not(all(windows, any(target_arch = "x86", target_arch = "x86_64"))))]
...
#[cfg(all(test, all(windows, any(target_arch = "x86", target_arch = "x86_64"))))]
#[path = "lib_tests.rs"]
mod tests;
