...
use crate::sys::{
    ...
    CUDA_VIDEO_SURFACE_FORMAT_NV12, CUDA_VIDEO_SURFACE_FORMAT_P016, CUVIDDECODECREATEINFO,
    CUVIDEOFORMAT, CUVIDPARSERDISPINFO, CUVIDPARSERPARAMS, CUVIDPICPARAMS, CUVIDPROCPARAMS,
    CUVIDSOURCEDATAPACKET, CUVID_PKT_ENDOFSTREAM, CUVID_PKT_TIMESTAMP,
};
...
const MAX_HOST_FRAME_BYTES: usize = 512 * 1024 * 1024;
...
struct CallbackState {
    ...
    display_left: u32,
    display_top: u32,
    /// Component depth selected for the current NVDEC output surface.
    bit_depth: u8,
...
unsafe extern "C" fn seq_callback(user_data: *mut c_void, fmt: *mut CUVIDEOFORMAT) -> i32 {
    ...
    let f = unsafe { &*fmt };
    if f.chroma_format != CUDA_VIDEO_CHROMA_FORMAT_420 as i32
        || !matches!(f.bit_depth_luma_minus8, 0 | 2)
        || f.bit_depth_luma_minus8 != f.bit_depth_chroma_minus8
    {
        guard.error = Some(format!(
            "NVDEC supports only matched 8/10-bit YUV420 output, received chroma {} and depths {}/{}",
            f.chroma_format,
            f.bit_depth_luma_minus8.saturating_add(8),
            f.bit_depth_chroma_minus8.saturating_add(8)
        ));
        return 0;
    }
    ...
    create.output_format = if f.bit_depth_luma_minus8 == 0 {
        CUDA_VIDEO_SURFACE_FORMAT_NV12
    } else {
        CUDA_VIDEO_SURFACE_FORMAT_P016
    };
    ...
    guard.bit_depth = f.bit_depth_luma_minus8.saturating_add(8);
...
unsafe extern "C" fn display_callback(
    ...
    // ── copy device → host ─────────────────────────────────────────
    // NV12/P016 layout: pitch * coded_h luma plane, immediately followed by
    // pitch * (coded_h / 2) interleaved UV plane. Pitch is always in bytes.
    let Some(y_pitched_len) = pitch.checked_mul(coded_h) else {
        guard.error = Some("NVDEC luma readback geometry overflowed".into());
        unsafe {
            let _ = (vt.cuvid_unmap_video_frame_64)(guard.decoder, dev_ptr);
        }
        return 0;
    };
    let Some(uv_pitched_len) = pitch.checked_mul(chroma_h) else {
        guard.error = Some("NVDEC chroma readback geometry overflowed".into());
        unsafe {
            let _ = (vt.cuvid_unmap_video_frame_64)(guard.decoder, dev_ptr);
        }
        return 0;
    };
    if match y_pitched_len.checked_add(uv_pitched_len) {
        Some(length) => length > MAX_HOST_FRAME_BYTES,
        None => true,
    } {
        guard.error = Some("NVDEC readback exceeds the host frame size limit".into());
        unsafe {
            let _ = (vt.cuvid_unmap_video_frame_64)(guard.decoder, dev_ptr);
        }
        return 0;
    }
    let mut y_pitched = vec![0u8; y_pitched_len];
    let mut uv_pitched = vec![0u8; uv_pitched_len];
    ...
    // ── crop to display rect, deinterleave NV12/P016 → planar 4:2:0 ──
    ...
    if out_w == 0 || out_h == 0 {
        guard.error = Some("NVDEC returned an empty display rectangle".into());
        return 0;
    }
    ...
    let bytes_per_sample = if guard.bit_depth > 8 { 2 } else { 1 };

    let Some(y_len) = out_w
        .checked_mul(out_h)
        .and_then(|samples| samples.checked_mul(bytes_per_sample))
    else {
        guard.error = Some("NVDEC luma output geometry overflowed".into());
        return 0;
    };
    let Some(chroma_len) = chroma_out_w
        .checked_mul(chroma_out_h)
        .and_then(|samples| samples.checked_mul(bytes_per_sample))
    else {
        guard.error = Some("NVDEC chroma output geometry overflowed".into());
        return 0;
    };
    let mut y_out = vec![0u8; y_len];
    let mut u_out = vec![0u8; chroma_len];
    let mut v_out = vec![0u8; chroma_len];

    for row in 0..out_h {
        let src_row = disp_top + row;
        let src_off = src_row * pitch + disp_left * bytes_per_sample;
        let row_bytes = out_w * bytes_per_sample;
        let dst_off = row * row_bytes;
        if src_off + row_bytes > y_pitched.len() {
            guard.error = Some("NVDEC luma crop exceeds the mapped surface".into());
            return 0;
        }
        y_out[dst_off..dst_off + row_bytes]
            .copy_from_slice(&y_pitched[src_off..src_off + row_bytes]);
    }

    for row in 0..chroma_out_h {
        let src_row = chroma_top + row;
        let src_off = src_row * pitch + chroma_left * 2 * bytes_per_sample;
        let dst_off = row * chroma_out_w * bytes_per_sample;
        let row_bytes = chroma_out_w * 2 * bytes_per_sample;
        if src_off + row_bytes > uv_pitched.len() {
            guard.error = Some("NVDEC chroma crop exceeds the mapped surface".into());
            return 0;
        }
        let row_data = &uv_pitched[src_off..src_off + row_bytes];
        for col in 0..chroma_out_w {
            let source = col * 2 * bytes_per_sample;
            let destination = dst_off + col * bytes_per_sample;
            if source + 2 * bytes_per_sample <= row_data.len() {
                u_out[destination..destination + bytes_per_sample]
                    .copy_from_slice(&row_data[source..source + bytes_per_sample]);
                v_out[destination..destination + bytes_per_sample].copy_from_slice(
                    &row_data[source + bytes_per_sample..source + 2 * bytes_per_sample],
                );
            }
        }
    }
    ...
    guard.frames.push_back(VideoFrame {
        ...
        planes: vec![
            VideoPlane {
                stride: out_w * bytes_per_sample,
                data: y_out,
            },
            VideoPlane {
                stride: chroma_out_w * bytes_per_sample,
                data: u_out,
            },
            VideoPlane {
                stride: chroma_out_w * bytes_per_sample,
