...
The native probe path parses MP4/MOV and Matroska/WebM structure, emits typed tracks, and has integration tests against generated fixtures. macOS VideoToolbox encode/decode probes are executable today. Retained OpenH264 software sessions provide H.264 decode and encode on macOS, Windows, Linux, and Linux-based NAS hosts on both x86-64 and ARM64. Linux and Windows hardware backend contracts are modeled with explicit capability states, but those hardware backends are not executable yet.
