...
The native probe path parses MP4/MOV and Matroska/WebM structure, emits typed tracks, and has integration tests against generated fixtures. macOS VideoToolbox encode/decode probes are executable today. Retained OpenH264 sessions provide H.264 decode and encode, safe Rust provides HEVC Main/Main10 decode, retained dav1d sessions provide AV1 decode, pure Rust provides Opus decode, and safe Rust encoders provide AAC-LC, AC-3, and E-AC-3 on macOS, Windows, Linux, and Linux-based NAS hosts on both x86-64 and ARM64. Linux release builds enable the `linux-vaapi` feature: H.264 and HEVC Main/Main10 packets are submitted through retained VA-API decoders into page-aligned NV12/P010 user-pointer surfaces, and BGRA frames can be encoded to H.264 or HEVC through retained VA-API sessions. Intel iHD/i965 render nodes are additionally exposed as Quick Sync backends only after Intel vendor detection and real codec probes succeed. libva is loaded at runtime, so systems and NAS hosts without a usable render node fall back to the portable codecs. Source builds need libdav1d 1.3+ discoverable through `pkg-config`; release bundles must include the corresponding native library.
