    ...
    for desc in packet_descriptions.iter().take(packet_count as usize) {
        let start =
            usize::try_from(desc.mStartOffset).map_err(|_| AudioEncodeError::BackendFailed {
                reason: "AudioToolbox returned a negative AAC packet offset".to_string(),
            })?;
        let size = desc.mDataByteSize as usize;
        let end = start
            .checked_add(size)
            .ok_or_else(|| AudioEncodeError::BackendFailed {
                reason: "AudioToolbox AAC packet range overflowed".to_string(),
            })?;
        let payload = data
            .get(start..end)
            .ok_or_else(|| AudioEncodeError::BackendFailed {
                reason: "AudioToolbox AAC packet range exceeded output buffer".to_string(),
            })?
            .to_vec();
